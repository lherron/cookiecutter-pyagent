...
class JMAPConfig(BaseModel):
    """JMAP configuration for Fastmail."""
    api_key: Optional[str] = Field(None, description="Fastmail API bearer token for Authorization header (Bearer {token})")
    agent_email_address: Optional[str] = Field(None, description="Your Fastmail email address")
    user_email_address: Optional[str] = Field(None, description="Your Fastmail email address")
    server_url: str = Field("https://jmap.fastmail.com", description="JMAP server URL")
    source_folder: str = Field("Triage", description="Folder to process emails from")
    archive_folder: str = Field("Archive", description="Folder to move processed emails to")
...
def _load_section_config(section_key: str, model: type[BaseModel], config_data: Dict[str, Any], env_prefix: str) -> Optional[BaseModel]:
    ...
    # Check for JMAP_API_KEY which is a special case for the bearer token
    if section_key == "jmap":
        api_key = _get_env_var("JMAP_API_KEY")
        if api_key:
            env_overrides["api_key"] = api_key
        
        # Add support for JMAP_HOST to override server_url
        jmap_host = _get_env_var("JMAP_HOST")
        if jmap_host:
            env_overrides["server_url"] = jmap_host
            
        # Direct support for USER_EMAIL_ADDRESS and AGENT_EMAIL_ADDRESS
        user_email = _get_env_var("USER_EMAIL_ADDRESS")
        if user_email:
            env_overrides["user_email_address"] = user_email
            
        agent_email = _get_env_var("AGENT_EMAIL_ADDRESS")
        if agent_email:
            env_overrides["agent_email_address"] = agent_email
    
    # Check for ANTHROPIC_API_KEY which is the direct environment variable name
    elif section_key == "llm":
        anthropic_api_key = _get_env_var("ANTHROPIC_API_KEY")
        if anthropic_api_key and "anthropic" not in env_overrides:
            if not section_data.get("anthropic"):
                env_overrides["anthropic"] = {}
            if isinstance(env_overrides.get("anthropic"), dict):
                env_overrides["anthropic"]["api_key"] = anthropic_api_key
            else:
                env_overrides["anthropic"] = {"api_key": anthropic_api_key}
                
            # If provider is not set, default to anthropic when the key is present
            if "provider" not in env_overrides and "provider" not in section_data:
...
def load_config(config_path: Optional[str] = None) -> AppConfig:
    ...
    jmap_config = _load_section_config("jmap", JMAPConfig, config_data, "JMAP")
...
if __name__ == "__main__":
    ...
    # os.environ["ANTHROPIC_API_KEY"] = "test_anthropic_key_from_env"  # Direct environment variable for Anthropic
    ...
    try:
        ...
        if config.jmap:
            print("\nJMAP Config Loaded:")
            print(f"  API Key Set: {'Yes' if config.jmap.api_key else 'No'}")
            print(f"  Agent Email Address: {config.jmap.agent_email_address or 'Not Set'}")
            print(f"  User Email Address: {config.jmap.user_email_address or 'Not Set'}")
            
            print(f"  Server URL: {config.jmap.server_url}")
            print(f"  Source Folder: {config.jmap.source_folder}")
            print(f"  Archive Folder: {config.jmap.archive_folder}")
        else:
            print("\nJMAP Config: Not Loaded/Disabled")
